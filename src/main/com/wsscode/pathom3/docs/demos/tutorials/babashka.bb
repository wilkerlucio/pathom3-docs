...
;
;#_(defn main [{:keys [ip] :as args}]
;    (let [sm (psm/smart-map env {::ip ip})]
;      (println (str "Temperature for IP " ip " is " (::temperature sm) "C"))))
;
;(p.eql/process env
;  {::ip "198.29.213.3"}
;  [::temperature])
...
;  (psm/smart-map env {::temperature 30})
;
;  (let [sm (psm/smart-map env {:meta-weather/woeid 23424768})]
;    (-> sm :meta-weather/the_temp)))

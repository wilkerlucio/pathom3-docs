...
:::note
If you are developing a product on a company, use the company name for the step one.
Avoid having short names like `:user/name` since they have higher chance of collision.
...
we make that happen providing the `:hacker-news.page/news-page-url` in that context; which can navigate
...
that is a flat structure. On top of the data I mentioned in the image, we now will
also add a `:hacker-news.comment/ident`, which tells us the indentation level of that
row. Later I'll use this transform the list in a tree.
...
Now it's time to transform the list in a tree.
...
current level, when it goes up, add an item to a stack, when goes down, remove from the
stack. The stack will contain the ids of the parent items. As I scan I'll also add
the items to a list, indexed by ID. This way I can modify any item any time with ease.

I decided would be fun to use Pathom as part of this process too, and it's nice the
...
Cool right?! You may be asking now: how do you I know what can I access?
...
There we can see a lot of data related to the item (given we provided an item id). Also,
...
:::tip
Value responses of smart maps are also smart maps. This is what allow the navigation
like you seen before. This also means you can datafy at any point to see what paths
...
This is the list of resolvers we have to deal with the news page:
...
parse it, like we did for comments and user details.
...
- The attribute names they are use for the collection node are different
...
Even more when we consider that the `ask` page will be another instance of the same
situation.

...
entity in TMDB is a generic type called `Node`, so in that point this schema isn't clear
about what kind of entity is there, and in this case Pathom will offer all the possible
options there.

An interesting part of this translation from Pathom, is that Pathom will automatically
group the attributes when sending to GraphQL using the `... on TYPE { attr attr2 } ` syntax,
and since in Pathom we have the full type name on each attribute that grouping is based
on looking at the last namespace part in the attribute name.
:::
...
:::note
In this scenario we see an N+1 issue, this is due to a current limitation around
batching with dynamic resolvers. In the future this should be optimized internally

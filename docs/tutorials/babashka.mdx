...
This tutorial will demonstrate how to use Pathom 3 in [Babashka](https://github.com/babashka/babashka).

For this, I'll re-use the sample example from the [Pathom Tutorial](tutorial.mdx), the code is mostly
...
On my machine this script runs in about 2s. We can compare it with the JVM version,
which takes about 9s on my machine, that makes the Babashka version close to 5 times
faster!

## Limitations

Smart Maps are not supported in Babashka at this time. This is due to some current
Babashka limitations, but I've been in touch with [Michiel Borkent](https://github.com/borkdude) and there are
some working going on Babashka and SCI that may allow this in the future.

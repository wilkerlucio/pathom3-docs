...
Some key points:
- We have to manually integrate each resolvers, which means every change on the server might require changes on the clients
...
services. It just won't scale, so we got to have a better way to handle this.

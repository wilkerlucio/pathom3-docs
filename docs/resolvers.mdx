...
Resolvers become interesting when you combine many of then, this is done via the `pci/register`
function.

From the resolvers you can generate the indexes. The Pathom engine
uses those indexes to plan and run the resolvers. For more details on indexes visit
the [indexes page](indexes.mdx).

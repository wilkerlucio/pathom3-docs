...
Scaffolding is done. Time to start writing some resolvers.
...
`https://www.metaweather.com/api/location/$WOEID$` endpoint (`WOEID` stands for [Where On Earth ID](https://en.wikipedia.org/wiki/WOEID))
...
longitude. In these systems, this means that now `WOEID` depends on `latitude` and `longitude`:
...
:::note
In `latlong->woeid` resolver, we let Pathom infer the output automatically. To use this
...
which dictates the step. Now we will to replace all the resolver names with a single
...
:::tip
While requesting information with Pathom, it helps to think in terms of the attribute
...
It depends on `:woeid`, which we don't have, but the index says you can
get it if you provide `:latitude` and `:longitude`. We don't have those either, then
Pathom has to look again in the index for then, and those are available through `:ip`,
...
is an execution graph that describes what will take to fulfill the demand. Then it
...
- Use [Smart Maps](smart-maps.mdx) to make the information request.

I designed this demo to illustrate the basic concepts of attribute modeling and Pathom.

I like to encourage you to go and play with it a bit more. Here are a few exercise suggestions:
